...
task<IRandomAccessStream^> ResourceLoader::LoadStorageFile(StorageFile^ storageFile)
{
	...
	{
		return continue_task(accessStream->ReadAsync(ref new Buffer(64 * 1024), 64 * 1024, InputStreamOptions::None),
...
task<void> ResourceLoader::WriteBufferToResultStream(IBuffer^ buffer, bool finished)
{
	...
	if (_cacheResult)
	{
		...
		if (_cacheWriter == nullptr)
		{
			...
			return continue_task(ApplicationData::Current->TemporaryFolder->CreateFileAsync(targetFileName, CreationCollisionOption::FailIfExists),
...
task<void> ResourceLoader::FailureCacheCleanup()
{
	...
	return create_task(Windows::Storage::ApplicationData::Current->TemporaryFolder->GetFileAsync(targetFileName))
		.then([=](task<StorageFile^> tsk)
	{
		try
		{
			auto file = tsk.get();
		...
		{
			//file didnt exist anyway nothing to do here
			return task_from_result();
		}	
	});
}
