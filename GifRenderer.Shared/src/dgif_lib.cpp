...
DGifSlurp(GifFileType *GifFile)
{
	...
	public:
		revertHelper(GifFileType *gifFile) : _gifFile(gifFile) {}
		~revertHelper()
		{
			//revert the stream to the last known good read
			READ(_gifFile, nullptr, -1);
		}
	};
	...
	private:
		...
		bool _done;
	public:
		revertExtensionHelper(GifFileType* gifFile) : _gifFile(gifFile), _extensionCount(gifFile->ExtensionBlockCount), _done(false)
		{
			
		}
		void markDone() { _done = true; }
		~revertExtensionHelper()
		{
			if (!_done && _gifFile->ExtensionBlockCount > _extensionCount)
			{
				for (int i = _extensionCount; i < _gifFile->ExtensionBlockCount; i++)
				{
					if (_gifFile->ExtensionBlocks[i].Bytes != nullptr)
					{
						free(_gifFile->ExtensionBlocks[i].Bytes);
					}
				}
				_gifFile->ExtensionBlockCount = _extensionCount;
			}
		}
	};
	...
	do {
		...
		{
			revertExtensionHelper revertHelper(GifFile);
		...
		//mark the stream for recovery if we need to wait for more data
		READ(GifFile, nullptr, -2);
